...
class Task(object):
    ...
    def __init__(self, job):
        ...
        self.execution_was_successful = None

    class TaskHook(object):
        def __init__(self, name, init_function, clean_function):
            self.name = name
            self.init_function = set_status_to_cleaning_on_error(
                "failed to execute TaskHook {} init function".format(self.name)
            )(init_function)
            self.clean_function = set_status_to_cleaning_on_error(
    ...
    @set_status_to_cleaning_on_error("failed to stop task")
    def stop(self, processor):
        # after task is executed, it can leave some data in last_run_history_record,
        # which lead to self.next_retry_ts != None, but job status will be STATUS_CANCELLED,
        # not STATUS_PENDING, therefore undesirable retry is impossible
    ...
    @set_status_to_cleaning_on_error("failed to update status")
    ...
    @set_status_to_cleaning_on_error("failed to check status is finished")
    ...
    @set_status_to_cleaning_on_error("failed to check status is failed")
    def failed(self, processor):
        return self._failed(processor)

    @set_status_to_cleaning_on_error("failed to start execution")
    ...
    def load(self, data):
        ...
        self.execution_was_successful = data.get('execution_was_successful', None)
    ...
    def dump(self):
        res = {
            ...
            'execution_was_successful': self.execution_was_successful,
            'run_history': [
    ...
    def set_status(self, status):
        ...
        if status == Task.STATUS_EXECUTING:
            ...
            if not self.cleaning_details.get(Task.UNCLEANED_DETAILS, {}):
                ...
                if not self.execution_was_successful:
                    self.execution_was_successful = False  # None -> False or False -> False
                    return
                else:
                    logger.error(
                ...
                self.cleaning_details.update(self.cleaning_details[Task.UNCLEANED_DETAILS])
    ...
    @clean_after_error(allow_retry=True)
    ...
    @clean_after_error(allow_retry=True)
    def update_task(self, processor):
        ...
        if self.failed(processor):
            ...
            self.execution_was_successful = True
            self.set_status(Task.STATUS_CLEANING)
