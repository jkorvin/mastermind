...
class Balancer(object):
    ...
    def break_couple(self, request):
        ...
        with sync_manager.lock(self.CLUSTER_CHANGES_LOCK, blocking=False):
            ...
            try:
                self.infrastructure.couple_break(couple_as_tuple=tuple(group_ids))
            except Exception as e:
                logger.error('Failed to remove couple {0} from groups {1} history: {2}'.format(couple_str, group_ids, str(e)))
                raise
            else:
